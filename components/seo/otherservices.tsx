...
const serviceToSlug = (service: string, slugMapping: Record<string, string>): string => {
  return slugMapping[service] || service.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
};

// Function to divide services into equal columns dynamically
const divideIntoColumns = (services: string[], columnCount: number): string[][] => {
  ...
  return (
    <section className="py-20 px-6 bg-gradient-to-b from-pink/20 to-white">
      <div className="container max-w-7xl mx-auto">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 ${config.columns === 3 ? 'lg:grid-cols-3' : config.columns === 4 ? 'lg:grid-cols-4' : 'lg:grid-cols-2'} gap-10`}>
          {serviceColumns.map((column, columnIndex) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: "-100px" }}
              transition={{ duration: 0.6, delay: columnIndex * 0.1, ease: "easeOut" }}
              className="space-y-4"
            >
              <ul className="space-y-3">
                {column.map((service) => (
                  <li key={service} className="flex items-center gap-3 text-blackbrown/90">
                    ...
                    <Link
                      href={`${config.baseUrl}${serviceToSlug(service, slugMapping)}`}
