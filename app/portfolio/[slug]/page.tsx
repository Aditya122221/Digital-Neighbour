...
function summarizeContent(input?: string, limit = 160) {
  if (!input) return "";
...
export async function generateMetadata({
  ...
  const { slug } = params;
  const currentProject = portfolio.find((project) => project.slug === slug);

  if (!currentProject) {
    return buildMetadata({
      title: "Case Study",
      description:
        "See how Digital Neighbour designs growth programs that deliver measurable outcomes.",
      path: `/portfolio/${slug}`,
      type: "article",
    });
  }
  ...
  const summary =
    summarizeContent(currentProject.content) ||
    (currentProject.metrics?.length
      ? `See how ${currentProject.logoText} achieved ${currentProject.metrics[0].value} ${currentProject.metrics[0].label.toLowerCase()} with Digital Neighbour.`
      : `Discover how Digital Neighbour partnered with ${currentProject.logoText} to drive growth.`);

  return buildMetadata({
    title: currentProject.headline,
    description: summary,
    path: `/portfolio/${slug}`,
    type: "article",
  });
    ...
    <main>
      ...
      <section className="relative w-full h-[60vh] md:h-[70vh] mt-16 md:mt-20">
        {currentProject.image && (
          <Image
            src={imageSrc}
            alt={currentProject.imageAlt || currentProject.headline}
            fill
        ...
        <div className="container max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 mb-8">
            {otherProjects.map((project) => {
              const otherImageSrc = project.image?.startsWith("/")
                ? project.image
                : `/${project.image}`;
                ...
                >
                  <div className="aspect-[16/10] w-full overflow-hidden rounded-t-xl bg-muted">
                    {project.image && (
                      <Image
                        src={otherImageSrc}
                        alt={project.imageAlt || project.headline}
                        width={640}
                        height={400}
